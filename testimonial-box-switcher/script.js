...
const userProfessionEl = document.querySelector(".user-profession");
...
function showReview() {
  userReviewEl.innerText = reviewData[current].text;
  userImgEl.src = reviewData[current].photo;
  userImgEl.alt = `${reviewData[current].name}'s image`;
  userNameEl.innerText = reviewData[current].name;
  userProfessionEl.innerText = reviewData[current].position;
  // progressbarEl.style.width = 0;
  current++;
  if (current >= reviewData.length) current = 0;
}
